...
- 沒有可定位的原生 session 或 export。使用本 packet + Git source + ISSUE-018 復原；不能聲稱完整對話／模型 context 已移轉。
- `docs/durable-task-engine.md` 是當前架構與 live acceptance 清單。
- `docs/known-issues.md` ISSUE-018 包含完整、targeted、中途失敗及限制；ISSUE-014／015／016 live 狀態保持未驗收。
...
- 暫存 log 在來源 `/tmp/herdr-phase1-targeted.log`、`/tmp/herdr-phase1-final-full.log`；可能不隨機器轉移。必要數字及原因已寫入 ISSUE-018，不能假設目的端有 `/tmp` log 或 `node_modules`／`dist`。
...
2. 在允許本機 Unix socket 的環境執行 `npm ci`、`npm run typecheck`、`npm run build`、`npm run lint`、原始 `npm test`；記錄真實結果，更新 ISSUE-018／ISSUE-007，不能沿用受限環境結論為全部通過。
