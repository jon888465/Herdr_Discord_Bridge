...
2026-09-21。原始碼已合併；新增 Phase 4 fixtures 通過，但完整 gate 仍有 ISSUE-007／021 失敗。真實 Discord／Herdr／CLI 驗收待完成，見 ISSUE-022。
...
自動化 fixtures／測試紀錄見 [ISSUE-022](known-issues.md)。所有 tests 只使用 temporary Git、fake Herdr/Discord、fake clock，不代表真實 provider quota 或模型接手成功。本輪未部署／重啟 Bridge，running version 未核對，舊訊息與舊工作不補送。
