...
Task persistence/reconciliation 與取消已接上同一 scheduler（見 ISSUE-020）；仍未完成自動 resume、Team 多問題佇列／續接、跨 bridge 全域租用、原生 PTY 串流 attach、Discord mirror、完整事件匯流排。CLI 若不遵守 marker 或畫面截斷，擷取會失敗而非假裝完整。未重啟的 bridge 仍執行旧版本；舊任務與訊息不自動補送。
